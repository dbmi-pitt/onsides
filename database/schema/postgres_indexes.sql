-- Indexes for OnSIDES PostgreSQL schema
-- Safe to run repeatedly (IF NOT EXISTS)

-- Ensure we target the onsides schema by default
SET search_path TO onsides, public;

-- product_adverse_effect
-- Primary key already indexes effect_id; adding targeted indexes used in filters/joins
CREATE INDEX IF NOT EXISTS idx_pae_match_method ON product_adverse_effect (match_method);
CREATE INDEX IF NOT EXISTS idx_pae_label_section ON product_adverse_effect (label_section);
CREATE INDEX IF NOT EXISTS idx_pae_effect_meddra_id ON product_adverse_effect (effect_meddra_id);
CREATE INDEX IF NOT EXISTS idx_pae_product_label_id ON product_adverse_effect (product_label_id);
...
CREATE INDEX IF NOT EXISTS idx_vmae_term_type ON vocab_meddra_adverse_effect (meddra_term_type);

-- vocab_rxnorm_ingredient
-- PK on rxnorm_id already exists; add name/type for lookups
CREATE INDEX IF NOT EXISTS idx_vri_name ON vocab_rxnorm_ingredient (rxnorm_name);
